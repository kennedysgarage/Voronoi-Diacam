...
abstract class BaseOAuth {
  ...
  public $request_headers = array('Expect:');
  
  /* OAuth Token */
  protected $token = null;
  /* SHA1 Method to use for signing (default is HMAC-SHA1 */
   ...
   * Debug helpers
   */
  public function lastStatusCode() { return $this->http_code; }
  ...
  public function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) {
      $this->init($consumer_key, $consumer_secret, $oauth_token, $oauth_token_secret);
  }
  ...
  public function get($url, $parameters = array()) {
    ...
    if ($this->format === 'json' && $this->decode_json) {
      $j = new JSON(JSON::JSON_LOOSE_TYPE);
      return $j->decode($response);
    }
    return $response;
  }
  ...
  protected function http($url, $method, $postfields = NULL) {
    ...
    curl_setopt($ci, CURLOPT_HTTPHEADER, $this->request_headers);
